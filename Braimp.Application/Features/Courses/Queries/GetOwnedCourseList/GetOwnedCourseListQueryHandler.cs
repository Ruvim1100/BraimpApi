...
{
    public class GetOwnedCourseListQueryHandler(
        IBraimpDbContext dbContext, IBlobStorageService blobStorageService, ICurrentUserService currentUserService)
        : IRequestHandler<GetOwnedCourseListQuery, PaginationResult<OwnedCourseLookupModel>>
    {
        public async Task<PaginationResult<OwnedCourseLookupModel>> Handle(
            GetOwnedCourseListQuery request,
            CancellationToken cancellationToken)
        {
            ...
            var resultItems = pageData
                .Select(course => new OwnedCourseLookupModel
                {
                    Id = course.Id,
                    Title = course.Title,
            ...
            return new OwnedCourseListResponse(
                resultItems,
                request.Page,
                request.PageSize,
                totalCount);
